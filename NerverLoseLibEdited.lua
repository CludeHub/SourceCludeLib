...
Sound2.Ended:Connect(function()
	...
	Dropdown.Position = UDim2.new(0.05, 0, 0.601014614, 0)
	Dropdown.Size = UDim2.new(0.9, 0, -0.351014495, 0)
	Dropdown.ZIndex = 5
	...
	TopBar.Position = UDim2.new(0.95, 0, 0.5, 0)
	...
	ValueText.AnchorPoint = Vector2.new(0.5, 0.5)
	ValueText.BackgroundTransparency = 1.000
	ValueText.Position = UDim2.new(0.358571023, 0, 0.5, 0)
	...
	DownBar:GetPropertyChangedSignal('Size'):Connect(function()
		DownBar.Visible = DownBar.Size.Y.Offset >= 4
	end)
	...
	local function auto_updatea()
		local targetSize = DropdownToggle and get_list_v4() or 0
		TweenService:Create(DownBar, TweenInfo.new(0.25, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {
			Size = UDim2.new(0.4, 0, 0, targetSize)
		}):Play()
	end
	...
	local function DropdownToggleEffect(val)
		...
		if val then
			...
			TweenService:Create(Icon, TweenInfo.new(0.25, easing, dir), {Rotation = 90}):Play()
			TweenService:Create(LabelText, TweenInfo.new(0.4, easing, dir), {TextTransparency = 0}):Play()
			...
			for i, v in ipairs(DownBar:GetChildren()) do
				if v:IsA("TextButton") then
					...
					TweenService:Create(v, TweenInfo.new(0.25, easing, dir, 0, false, i * 0.02), {
						Size = UDim2.new(0.99, 0, 0.5, 0),
			...
			for i, v in ipairs(DownBar:GetChildren()) do
				if v:IsA("TextButton") then
					TweenService:Create(v, TweenInfo.new(0.15, easing, dir, 0, false, i * 0.015), {
			...
			TweenService:Create(Icon, TweenInfo.new(0.25, easing, dir), {Rotation = -90}):Play()
			TweenService:Create(LabelText, TweenInfo.new(0.4, easing, dir), {TextTransparency = 0.3}):Play()

			TweenService:Create(DownBar, TweenInfo.new(0.2, easing, dir), {
	...
	local function newbutton()
		...
		btn.Size = UDim2.new(0.99, 0, 0.5, 0)
		btn.ZIndex = 1000
		...
		TweenService:Create(btn, TweenInfo.new(0.1), {TextTransparency = 0.3}):Play()

		UIAR.Parent = btn
		UIAR.AspectRatio = 4.5
		UIAR.AspectType = Enum.AspectType.ScaleWithParentSize
